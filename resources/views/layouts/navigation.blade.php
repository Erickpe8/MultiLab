<nav x-cloak x-data="{ userManagementOpen: false }" class="fixed inset-y-0 left-0 z-40 w-64
           ...
           backdrop-blur-sm" :class="{ 'translate-x-0': sidebarOpen }">

    {{-- HEADER / LOGO + TEMA --}}
    <div class="h-16 px-4 flex items-center justify-between border-b border-[var(--border)]
               bg-gradient-to-r from-[var(--primary)]/5 to-transparent">
        <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 group">
            <img src="{{ asset('images/multilab.svg') }}" alt="Logo MultiLab"
                class="h-8 w-auto transition-transform duration-300 group-hover:scale-105" />
            <span class="font-semibold text-sm text-[var(--text)] hidden sm:inline">
                MultiLab
            </span>
        </a>
    ...
    <div class="px-4 py-4 border-b border-[var(--border)]">
        @php
            $user = Auth::user();
            $roleLabel = strtoupper(optional($user->roles->first())->name ?? 'SIN ROL');
        @endphp
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 space-y-3">
        ...
        <div>
            ...
            <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium
    ...
    : 'hover:bg-[var(--border)]/20 text-[var(--text)]' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 transition-transform duration-200 group-hover:scale-110
                           {{ request()->routeIs('dashboard') ? 'text-white' : 'text-[var(--accent)]' }}" fill="none"
        ...
        @if($user && $user->hasRole('superadmin'))
            <div class="pt-2">
                ...
                <div class="relative">
                    <button @click="userManagementOpen = !userManagementOpen" class="w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-medium
                                   transition-all duration-200 group
            ...
            : 'hover:bg-[var(--border)]/20 text-[var(--text)]' }}">
                        ...
                        <svg class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-180': userManagementOpen }"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>

                    {{-- SUBMENÚ CONTROL DE USUARIOS --}}
                    <div x-show="userManagementOpen" x-transition:enter="transition ease-out duration-200"
                        ...
                        <a href="{{ route('user-management.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm
                                       transition-all duration-200 group
        ...
        {{-- MÓDULOS (PLACEHOLDER MULTILAB) --}}
        <div class="pt-2">
            <p class="uppercase text-[10px] tracking-[0.15em] text-[var(--text)]/50 px-3 mb-2 font-bold">
                Módulos
            </p>
            <div class="mx-3 px-3 py-3 rounded-lg text-xs italic
                       bg-gradient-to-br from-[var(--border)]/10 to-transparent
                       border border-[var(--border)]/30
                       text-[var(--text)]/60">
                <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-[var(--accent)]/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span>
                        Próximamente: Inventario, Préstamos, Incidencias y Mantenimientos.
                    </span>
                </div>
            </div>
        </div>

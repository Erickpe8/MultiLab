...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Navegación de paginación"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-4 flex-1">
            ...
            <div class="flex gap-2 sm:hidden">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" class="inline-flex items-center px-3 py-2 text-sm font-medium rounded-lg
                ...
                @if ($paginator->hasMorePages())
                    <a href="{{ $paginator->nextPageUrl() }}" class="inline-flex items-center px-3 py-2 text-sm font-medium rounded-lg
                                      text-[var(--text)] bg-[var(--background)] border border-[var(--border)]
                                      hover:bg-[var(--border)]/10 transition-colors">
                        Siguiente
                    </a>
        ...
        <div class="hidden sm:flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <select id="per-page-{{ $pageName }}"
                    ...
                    onchange="this.form?.submit() || (window.location.href = window.updateQueryStringParameter(window.location.href, this.name, this.value))"
            ...
            <div class="flex items-center rounded-lg shadow-sm border border-[var(--border)] overflow-hidden">
                ...
                @foreach ($paginator->getUrlRange(1, $paginator->lastPage()) as $page => $url)
                    ...
                    @elseif ($page == 1 || $page == $paginator->lastPage() || abs($page - $paginator->currentPage()) <= 2)
                        <a href="{{ $url }}" class="px-4 py-2 text-sm font-medium bg-[var(--background)] text-[var(--text)]
                                              hover:bg-[var(--border)]/10 transition-colors
            ...
            <form method="GET" class="flex items-center gap-2">
                ...
                <input type="number" name="{{ $pageName }}" min="1" max="{{ $paginator->lastPage() }}"
                    value="{{ $paginator->currentPage() }}" class="w-16 px-2 py-1 text-sm text-center rounded-lg border border-[var(--border)]
                                  bg-[var(--background)] text-[var(--text)]
                                  focus:ring-2 focus:ring-[var(--primary)] focus:border-transparent"
    ...
    <!-- JavaScript helper para actualizar query strings (definido una sola vez) -->
    <script>
        if (typeof window.updateQueryStringParameter === 'undefined') {
